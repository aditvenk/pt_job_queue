...
def _build_commit_message(
    ...
    *,
    ...
    reference = issue_reference or (
        f"#{issue_number}" if issue_number is not None else ""
    )
    if reference:
        parts.append("")
        parts.append(f"Fixes {reference}")
    return "\n".join(parts).strip() + "\n"
...
def _existing_open_pr_url(
    ...
    *,
    ...
    if (
        job.pr_url
        and (github_repo is None or _github_repo_from_pr_url(job.pr_url) == github_repo)
        and get_pr_state(backend, job.pr_url, force_refresh=True) == "open"
    ):
        return job.pr_url
...
def _github_repo_from_pr_url(pr_url: str) -> str:
    parsed = _parse_github_pr_url(pr_url)
    if not parsed:
        return ""
...
def _target_profile_name_for_job(
    job: JobRecord,
    backend: Backend,
    job_dir: str,
    default_profile_name: str,
) -> str:
    if default_profile_name == "pytorch":
        return default_profile_name

    pytorch_diff = _read_file(backend, f"{job_dir}/pytorch-fix.diff")
    if pytorch_diff.strip():
        default_profile = get_profile(default_profile_name)
        default_worktree = f"{job_dir}/{default_profile.dir_name}"
        primary_status = backend.run(
            f"cd {default_worktree} && git status --porcelain",
            check=False,
        )
        primary_changes = (
            primary_status.returncode == 0 and bool(primary_status.stdout.strip())
        )
        if primary_changes:
            raise PtqError(
                "Solver changed both the primary repo and the PyTorch support "
                "worktree. PTQ cannot create one PR across two GitHub repos; split "
                "the changes or clean the unrelated repo before pushing."
            )
        return "pytorch"

    if job.pr_url:
        pr_repo = _github_repo_from_pr_url(job.pr_url)
        if pr_repo:
            profile_name = profile_name_for_github_repo(pr_repo)
            if profile_name:
                return profile_name
...
def _pr_branch_for_job(job: JobRecord, job_id: str, target_profile_name: str) -> str:
    if job.issue is None:
        return f"ptq/{job_id}"
    if target_profile_name != job.repo:
        return f"ptq/{job.repo}-{job.issue}"
    return f"ptq/{job.issue}"


def _issue_reference_for_pr(job: JobRecord, target_profile_name: str) -> str:
    if job.issue is None:
        return ""
    issue_profile = get_profile(job.repo)
    target_profile = get_profile(target_profile_name)
    if issue_profile.github_repo == target_profile.github_repo:
        return f"#{job.issue}"
    return f"{issue_profile.github_repo}#{job.issue}"
...
def sync_existing_pr_branch(
    ...
    *,
    ...
    target_profile_name = _target_profile_name_for_job(
        job,
        backend,
        job_dir,
        job.repo,
    )
    profile = get_profile(target_profile_name)
    worktree = f"{job_dir}/{profile.dir_name}"
    branch = _pr_branch_for_job(job, job_id, target_profile_name)
...
def create_pr(
    ...
    *,
    ...
    profile = get_profile(target_profile_name)
    worktree = f"{job_dir}/{profile.dir_name}"
    issue_reference = _issue_reference_for_pr(job, target_profile_name)
    existing_open_pr_url = ""

    if job.pr_url:
        if _github_repo_from_pr_url(job.pr_url) != profile.github_repo:
            _log(
                f"Stored PR targets {job.pr_url}, but current fix targets "
                f"{profile.github_repo}. Creating/finding a PR for the fix repo."
            )
        else:
            ...
            match pr_state:
                ...
                case "closed" | "merged":
                    _log(
                        f"Stored PR is {pr_state}. Creating a new PR from "
                        "current branch."
                    )
                case _:
                    _log(
                        "Stored PR state is unknown. Proceeding with "
                        "create-or-find flow."
                    )
