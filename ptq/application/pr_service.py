...
def _is_ptq_managed_comment(body: str) -> bool:
    ...
    return (
        ...
        or stripped.startswith(_BOT_COMMENT_PREFIX)
...
def fetch_pr_feedback(
    ...
    *,
    ...
    """Return human GitHub PR feedback for an existing open draft PR, if any."""
    ...
    summary_parts = []
    if comments:
        summary_parts.append(f"{len(comments)} human GitHub PR feedback item(s)")
    if ci_failures:
        summary_parts.append(f"{len(ci_failures)} failing PR check(s)")
    summary = (
        f"Found {' and '.join(summary_parts)}. Address human comments and inspect "
...
def create_pr(
    ...
    *,
    ...
    pr_title = title or f"Fix from PTQ job {job_id}"
